...
FishModelDawn::FishModelDawn(const Context* context, Aquarium* aquarium, MODELGROUP type, MODELNAME name, bool blend)
...
void FishModelDawn::init()
{
    ...
    inputState = contextDawn->createInputState({
        {0, 0, dawn::VertexFormat::FloatR32G32B32A32, 0},
    ...
    fishVertexBuffer = contextDawn->createBufferFromData(&fishVertexUniforms, sizeof(fishVertexUniforms), dawn::BufferUsageBit::TransferDst | dawn::BufferUsageBit::Uniform);
    ...
    fishPerBuffer = contextDawn->createBuffer(sizeof(fishPerUniforms), dawn::BufferUsageBit::TransferDst | dawn::BufferUsageBit::Uniform);
    ...
    if (skyboxTexture && reflectionTexture) {
        bindGroupModel = contextDawn->makeBindGroup(groupLayoutModel, {
            { 0, fishVertexBuffer, 0, sizeof(FishVertexUniforms) },
    ...
    bindGroupPer = contextDawn->makeBindGroup(groupLayoutPer, {
        { 0, fishPerBuffer, 0, sizeof(FishPerUniforms) },
    });
...
{
    contextDawn->setBufferData(fishVertexBuffer, 0, sizeof(FishVertexUniforms), &fishVertexUniforms);
}
...
void FishModelDawn::updatePerInstanceUniforms() const
{
    contextDawn->setBufferData(fishPerBuffer, 0, sizeof(fishPerUniforms), &fishPerUniforms);
}

void FishModelDawn::updateFishCommonUniforms(float fishLength, float fishBendAmount, float fishWaveLength)
{
...
void FishModelDawn::updateFishPerUniforms(float x, float y, float z, float nextX, float nextY, float nextZ, float scale, float time)
{
